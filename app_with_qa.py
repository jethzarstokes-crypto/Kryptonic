...
def load_index(path: str):
    return json.loads(SysPath(path).read_text(encoding="utf-8"))

def find_answer(user_q: str, qa_map: dict, cutoff: float = 0.6):
...
class CryptoChatbot:
    ...
    def __init__(self, gemini_api_key):
        ...
        self.qa_index = {}
        try:
            self.qa_index = load_index("qa_index.json")
        except Exception as e:
            print(f"Warning: Could not load qa_index.json: {e}")
...
Remember: Crypto prices change super fast! ⚡"""
    ...
    def process_query(self, user_input, language_code, conversation_history):
        ...
        # Q&A Index check before CSV or Gemini
        ans, matched_q, score = find_answer(user_input, self.qa_index, cutoff=0.6)
        if ans and score >= 0.6:
            response = f"📚 **Matched Question:** {matched_q}\n\n{ans}\n\n(confidence: {score:.2f})"
